...
| `AIOS/history/chat-history/claude-code/` | **Generated automatically** (optional), by `backup-claude-code.py` — Claude Code CLI session transcripts as readable Markdown. A separate session store from Cowork/Claude Desktop, same shape. | **No — machine-written** |
| `AIOS/history/chat-history/claude-code-raw/` | **Generated.** The same sessions exactly as Claude Code stored them (`.jsonl`). | **No — machine-written** |
...
| `AIOS/history/screenshots/` | **Generated by `shot.py`.** Every screenshot you register, renamed to a dated convention, with a sidecar note next to each one and a browsable index (`Screenshots.md`). The facts are also embedded inside the image file itself, so they survive it leaving the vault. | Yes — via `shot.py` only |
...
| `AIOS/history/transcripts/` | **Generated by `video.py`.** Every video you send, as text — one `<id>.md` per video, header with title/channel/date/length, then the full transcript. Raw material, not notes; what a video *means* goes in `Atlas/Media/`. | **No — machine-written** |
| `AIOS/generated/` | **Generated, never hand-edited.** `git-status.md` (is the optional git backup committed/pushed), `where.md` (one-grep note index), `commands.md` (every trigger phrase, flat), `taste.md` (every `## Taste` section, compiled), `happened.md` (every diary event, indexed), `scale.md` (per-folder note counts), `catalog.md` (every show/game/book by status, plus worlds/servers and purchases, one file — via `catalog.py`). | **No — machine-written** |
...
| `Atlas/People/` | **One note per specific family member, friend or teacher** — `Atlas/People/<Name>.md`, via `person.py`. Where `Family.md`/`Friends and social.md`-style notes describe the shape of those relationships, this is per-person detail: what's known about *that one person*, dated as it comes up. Index: `Atlas/People/People.md`, grouped family/friend/teacher, machine-written. Nothing from `Privat/`. | Yes — via `person.py` |
...
| `Atlas/Research/` | One note per question that was actually looked up — answer, why, source table, what would change it. Check here before researching the same thing twice. | Yes |
...
| `Atlas/Links/` | **One note per URL sent** — the link, the site, what the page is, tags, date, status (`unread`/`read`/`useful`/`dead`), plus an empty `## My notes` section for you. **Never write one by hand:** `python3 AIOS/scripts/link.py "<url>"` fetches the page and writes the note. The notes are the truth; `Links.md`'s table is generated from them (`--rebuild`), and `Links.base` is the Obsidian view. Everything from `## My notes` down is yours and is never rewritten — not on update, not on `--check`. | Yes — via `link.py` |
...
| **What a video actually said** | `AIOS/history/transcripts/<id>.md`, or run `python3 AIOS/scripts/video.py "<url>"`. **Never describe a video that hasn't been run through this** — it exits non-zero rather than let you guess. `--list` shows what's already cached |
| **A link I sent — "that thing I sent you", a site, a video, an article** | `Atlas/Links/` — one note per URL, with its real title. `python3 AIOS/scripts/link.py --find "<word>"` searches them; `--list` shows the newest |
| **A specific family member, friend, or teacher — what's known about *them*** | `Atlas/People/<Name>.md`. Check here before assuming a fact about someone isn't saved |
...
| What accounts I have and what I'm paying for | `Atlas/Reference/My online accounts.md` — `python3 AIOS/scripts/accounts-audit.py --check` ages what's unverified |
| **What games/shows/books/movies I'm into and their status, or what I've bought / am deciding to buy** | `AIOS/generated/catalog.md` — grep it before opening every `Atlas/Media/` note |
| My real balance, spending, ledger | `Atlas/About Me/Money.md` — `python3 AIOS/scripts/money.py` logs a transaction or sets a real balance |
| Screen time / sleep numbers | `Atlas/About Me/Screen time.md` — `python3 AIOS/scripts/screentime.py --avg` for the rolling average |
| Disk/RAM trend on a machine | `AIOS/history/machine-snapshots.md` — one dated row per `machine-snapshot.py` run |
| A screenshot I sent — what it showed, where it came from | `AIOS/history/screenshots/Screenshots.md` and the sidecar note next to each image. New ones only via `shot.py` |
| Did we already research this | `Atlas/Research/<Question>.md` — check before researching anything again |
| Claude Code CLI sessions (terminal, not Cowork/Claude Desktop) | `AIOS/history/chat-history/claude-code/` — generated by `backup-claude-code.py`, don't edit |
