...
| **Anyone from family, friends, or teachers — first mention or a new detail** | **Their own note, immediately, no threshold.** `python3 AIOS/scripts/person.py "<Name>" --category family/friend/teacher --relation "..." --fact "..."` — creates `Atlas/People/<Name>.md` on the first mention, appends a dated fact on every later one. Unnamed still gets a note under a role (`Mum`, `Maths teacher`) rather than waiting for a name. Any other person who keeps coming up (not family/friend/teacher, e.g. a coworker or a name from a video) still gets the same treatment via the same script — `--category other`. Nothing from `Privat/`. |
